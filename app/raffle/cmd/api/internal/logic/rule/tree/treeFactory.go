...
const ruleTreeCache = "ruleTree:%d"
...
func (d DefaultTreeFactor) OpenLogicTree(strategyAward *model.StrategyAward) *DecisionTreeEngine {
	// 1. 初始化
	cacheKey := fmt.Sprintf(ruleTreeCache, strategyAward.StrategyId)
	var ruleTree *RuleTree

	// 2. 尝试从缓存中获取数据
	treeJSON, err := d.svcCtx.BizRedis.Get(cacheKey)
	if err == nil && treeJSON != "" {
		err := json.Unmarshal([]byte(treeJSON), &ruleTree)
		if err == nil {
			// 缓存命中，直接返回
			...
			logx.Error("Failed to unmarshal ruleTree from JSON", err)
		}
	}

	// 3. 缓存未命中，执行原始逻辑从数据库加载
	...
	for _, node := range nodes {
		vo := &RuleTreeNode{
			...
			RuleDesc:           node.RuleDesc,
			RuleValue:          node.RuleValue.String,
			TreeNodeLineVOList: treeNodeLineMap[node.RuleKey],
		}
	...
	// 8. 缓存树结构
	cache, err := json.Marshal(&ruleTree)
	if err != nil {
		logx.Error("Failed to marshal ruleTree to JSON", err)
	}
	threading.GoSafe(func() {
		err := d.svcCtx.BizRedis.Set(cacheKey, string(cache))
		if err != nil {
			logx.Error("Failed to set ruleTree JSON to Redis", err)
		}
	})
